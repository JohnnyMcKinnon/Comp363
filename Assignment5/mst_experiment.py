...
def make_graph(n, density, max_w, seed):
    ...
    # make random spanning tree so graph is connected
    order = list(range(n))
...
def main():
    ...
    for n in sizes:
        ...
        for t in range(trials):
            ...
            G = make_graph(n, density, max_weight, t)
            ...
            # correctness
            if total_weight(Tb) != total_weight(Tk):
                print("Error: MST weights don't match")
                return

        avg_b = statistics.mean(b_times)
        avg_k = statistics.mean(k_times)

        results.append((n, avg_b, avg_k))

        print("n =", n,
              " Boruvka avg =", round(avg_b, 5),
              " Kruskal avg =", round(avg_k, 5))

    # simple plot
    xs = [r[0] for r in results]
    ys_b = [r[1] for r in results]
    ys_k = [r[2] for r in results]

    plt.plot(xs, ys_b, marker='o', label="Boruvka")
    plt.plot(xs, ys_k, marker='o', label="Kruskal")
    plt.xlabel("n")
    plt.ylabel("runtime (seconds)")
    plt.legend()
    plt.title("MST Timing Comparison")
    plt.show()
...
#Report
"""
Minimum Spanning Tree Performance Comparison
Boruvka vs Kruskal


1- Setup
Graphs were generated as symmetric adjacency matrices using 0 as no-edge. 
All edge weights were positive integers in range [1, 50]. 
To ensure that every graph is connected, I first created
a random spanning tree by connecting each vertex to a previously
visited vertex. After ensuring connectivity, additional edges were
added independently with probability equal to the density parameter (0.4).

The experiment tested graph sizes:
n = 10, 20, 30, 40, 50

8(lucky number)independent trials were run. In each runthrough, a graph
was generated once and then both algorithms (Boruvka and Kruskal)
were executed on the exact same graph to compare equally.

Timing--> time.perf_counter()


2- Results
The runtime of Boruvka increased rapidly as n increased, while the
runtime of Kruskal grew significantly slower.

Boruvka showed steep growth as graph size increased.
Kruskal showed comparatively moderate growth.
...
The plot of runtime versus n proves Boruvka’s curve rises
much more than Kruskal’s.


3- Discussion
Observed performance matches theoretical expectations.

Boruvka repeatedly scans entire adjacency matrix while also
recomputing connected components at each iteration. Since graph
is stored as a matrix and implementation checks all pairs of
vertices repeatedly, this results in quartic growth.

Kruskal builds an edge list once, sorts it, and
uses an efficient 'union-find' structure with path
compression and union by rank. This dramatically reduces redundant
work and prevents repeated full scans of the matrix O(n).

As n increases, the difference between the algorithms becomes more
obvious. Around n=50, Boruvka becomes noticeably slower, while
Kruskal remains practical.

If larger sizes were tested, Boruvka’s runtime would likely become
less ideal due to steep growth rate.


4- Threats to Validity / Limitations
a) Python interpreter overhead affects timing.
b) Small values of n won't fully reflect asymptotic growth
c) The adjacency matrix representation isn't ideal for Kruskal in
real world implementations. Edge lists are better.
d) Only one density value (at 0.4) was tested.
Despite these limitations, the overall trend is clear and consistent
with theoretical complexity analysis.


5- Conclusion
The experiment demonstrates how algorithmic design strongly impacts
practical performance. Although both Boruvka and Kruskal compute the
same minimum spanning tree, Kruskal scales significantly better in
this implementation.

The experimented data strongly supports theoretical complexity
analysis that Boruvka grows much faster than Kruskal as the graph size
increases [n-->infinity].
"""